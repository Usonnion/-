...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    NSInteger imageFlowMaxSelectedNumber = [LoadingManager sharedManager].maxLoadingImageCount;
    if (self.selectedAssetsArray.count >= imageFlowMaxSelectedNumber) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"alertTitle", @"DNImagePicker", nil) message:[NSString stringWithFormat:NSLocalizedStringFromTable(@"alertContent", @"DNImagePicker", nil), @(imageFlowMaxSelectedNumber)] delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"alertButton", @"DNImagePicker", nil) otherButtonTitles:nil, nil];
        [alert show];
